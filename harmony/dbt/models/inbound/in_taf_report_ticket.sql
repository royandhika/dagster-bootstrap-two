...
with queue as (
    select * 
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'inbound_taf_cc_queue_intelix') }}
    where is_ticket = 2
        {% if is_incremental() %}
...
,queue_detail as (
    select * 
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'inbound_taf_cc_queue_detail_intelix') }} a
    where exists (
        select ticket_id 
        from queue b
...
,master_category as (
    select * 
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'inbound_taf_cc_master_category_intelix') }}
)
,master_reference as (
    select * 
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'inbound_taf_cc_master_reference_intelix') }}
)
,master_customer as (
    select * 
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'inbound_taf_cc_master_customer_intelix') }} a
    where exists (
        select customer_id 
        from queue b
