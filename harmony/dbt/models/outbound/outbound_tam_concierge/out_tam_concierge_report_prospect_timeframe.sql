...
with src as (
    select
        *
        ,rn = row_number() over(partition by interaction_id order by uploaddate desc)
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'outbound_tam_concierge_report_prospect_timeframe_api') }}
	{% if is_incremental() %}
