{{
	config(
        ...
        unique_key=['data_supply_id', 'call_type'],
...
,attempt as (
	select 
        *
		,last_response = row_number() over(partition by data_supply_id, case 
		...
		,rn = row_number() over(partition by data_supply_id, case 
			when interaction_date <= interest_date then 'regular'
			when interaction_date > interest_date then 'follow_up'
		end order by interaction_date desc)
		,call_type = case 
			when interaction_date <= interest_date then 'regular'
			when interaction_date > interest_date then 'follow_up'
		end
