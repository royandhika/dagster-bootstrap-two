{{
	config(
		...
		unique_key=['agreement_no', 'interaction_id'],
...
with src as (
    select
        *
        ,rn = row_number() over(partition by agreement_no, interaction_id order by uploaddate desc)
    from {{ source(env_var('ENV_SCHEMA') + '_dl', 'outbound_clipan_duitcair_report_prospect_timeframe_api') }}
	{% if is_incremental() %}
