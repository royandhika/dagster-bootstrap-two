...
,campaign_result as (
    select 
        ...
        ,campaign_id 
        ,file_id
        ,max(case when field_name_id in ('q1_reconfirm_jadwal_cek_ecu') then data_content end) as [q1_reconfirm_jadwal_cek_ecu]
        ,max(case when field_name_id in ('q2_reschedule_service') then data_content end) as [q2_reschedule_service]
        ,max(case when field_name_id in ('q2a_reschedule_service') then data_content end) as [q2a_reschedule_service]
        ,max(case when field_name_id in ('q3_closing') then data_content end) as [q3_closing]
        ,max(case when field_name_id in ('q3a_closing') then data_content end) as [q3a_closing]
        ,max(case when field_name_id in ('q3b_closing') then data_content end) as [q3b_closing]
...
,final as (
    select
        ...
        ,b.[equipmentno]
		,c.campaign_id
        ,c.[q1_reconfirm_jadwal_cek_ecu]
        ,c.[q2_reschedule_service]
        ,c.[q2a_reschedule_service]
        ,c.[q3_closing]
        ,c.[q3a_closing]
        ,c.[q3b_closing]
		,d.[connect_response]
