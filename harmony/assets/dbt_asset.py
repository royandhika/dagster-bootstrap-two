...
from shared.partitions import partition_3hourly, partition_12hourly, partition_daily
import json
...
def ops_datamart_daily(context: AssetExecutionContext, dbt: DbtCliResource, config: CustomDbtRun):
    start, end = context.partition_time_window
    dbt_vars = {
        "min_date": start.strftime('%Y-%m-%d %H:%M:%S'), 
        "max_date": end.strftime('%Y-%m-%d %H:%M:%S')
